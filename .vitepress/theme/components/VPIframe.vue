<template>
  ...
  <div v-if="url" class="demo-model" :class="{
    'collapsed': !expanded,
    ...
    <div class="demo-header">
      ...
      <i class="i-carbon-launch text-2xl demo-link" @click="openUrl" :style="`${expanded ? '' : 'height:0;width:0;opacity:0'}`"></i>
      <div class="expand-icon" style="cursor: pointer;" @click="toggleExpand">
        <i :class="expanded ? 'i-ep-fold' : 'i-ep-expand'" class="text-2xl"></i>
      </div>
    </div>
    <!-- iframe 容器 -->
    <div class="iframe-container">
      <iframe v-if="expanded&&transitionEnd" ref="iframe" id="demo" class="iframe" scrolling="auto" frameborder="0" :src="url" />
    </div>
  </div>
</template>
